...
class ResidentRepository:
    ...
    def list_all(self) -> list[Resident]:
        return (
            ...
            .filter(Resident.monitoring_status != "DELETED")
            .order_by(Resident.id)
            .all()
        )

    def list_with_inactive(self) -> list[Resident]:
    ...
    def update(self, resident: Resident, values: dict) -> Resident:
    ...
    def soft_delete(self, resident: Resident) -> Resident:
        resident.monitoring_status = "DELETED"
        self.db.commit()
        self.db.refresh(resident)
        return resident

    def add_device_if_missing(self, resident_id: str) -> None:
        device_id = f"DEV-{resident_id}"
        exists = self.db.query(Device.id).filter(Device.device_id == device_id).first()
        if not exists:
            self.db.add(Device(device_id=device_id, resident_id=resident_id))
            self.db.commit()

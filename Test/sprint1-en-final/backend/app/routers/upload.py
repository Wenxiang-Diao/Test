...
async def upload_bed_events(
    ...
    return UploadResult(inserted=count, message=f"Successfully imported {count} bed events")
...
async def upload_vitals(
    ...
    AlertRuleService(db).evaluate_all_residents()
    return UploadResult(inserted=count, message=f"Successfully imported {count} vital sign records")
...
async def upload_sleep_summary(
    ...
    AlertRuleService(db).evaluate_all_residents()
    return UploadResult(inserted=count, message=f"Successfully imported {count} sleep summaries")

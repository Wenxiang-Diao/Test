...
export default function VitalSignCharts({ data }: Props) {
  ...
  return (
    <div className="card">
      <h3 style={{ marginBottom: 16 }}>Vital Sign Trends</h3>
      {chartData.length === 0 ? (
        <p style={{ color: '#64748b' }}>No heart-rate or breathing-rate samples recorded for this dashboard window</p>
      ) : (
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        <div>
          <p style={{ fontSize: 13, color: '#64748b', marginBottom: 8 }}>Heart Rate (bpm)</p>
        ...
        <div>
          <p style={{ fontSize: 13, color: '#64748b', marginBottom: 8 }}>Breathing Rate (/min)</p>
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={chartData}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="time" tick={{ fontSize: 11 }} />

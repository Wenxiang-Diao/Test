...
│  日期选择：◀ 2026-06-09 ▶                                │
├──────┬──────┬──────┬──────┬──────┬──────┬──────┬───────┤
│评分  │睡眠时长│清醒  │效率  │离床  │最长离│平均  │平均   │
│ 78  │432分  │42分  │86%  │2次  │床12分│心率  │呼吸率 │
│     │       │      │     │     │      │72bpm │16次/分│
├─────────────────────────────────────────────────────────┤
...
This system is a dashboard prototype for aged-care staff and family members. The primary interaction pattern is view-filter-acknowledge. The five storyboards below cover all major functional flows. The navigation structure between pages is shown in the diagram above.
...
The resident sleep dashboard is the main information page for a single resident. The top section shows current status; the middle section shows tonight's sleep summary; the bottom section holds trend charts and historical comparison.
...
The **bed-exit timeline** is a horizontal time axis covering tonight's sleep window. Each out-of-bed interval is marked with start time, end time, and duration. Intervals that exceed the resident's baseline are highlighted in a distinct colour.
...
Below the table, the `explanation` field is displayed in plain text — for example: "This resident typically gets up between 02:00 and 03:00. Current `activity_status` has shifted from STATIC to ACTIVE, and heart rate is approximately 8 bpm above baseline."
...
The report page is aimed at carers and family members who want to review sleep trends. It offers a daily view and a weekly view.

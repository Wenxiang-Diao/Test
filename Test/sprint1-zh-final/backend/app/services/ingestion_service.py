...
class IngestionService:
    ...
    async def upload_bed_events(self, file: UploadFile) -> int:
        ...
        for _, row in df.iterrows():
            ...
            if not self.resident_repo.exists(resident_id):
                raise HTTPException(status_code=400, detail=f"居民 {resident_id} 不存在")
            ...
            if bed_status not in VALID_BED_STATUS:
                raise HTTPException(status_code=400, detail=f"无效 bed_status: {bed_status}")
            if activity not in VALID_ACTIVITY:
                raise HTTPException(status_code=400, detail=f"无效 activity_status: {activity}")
            try:
                ts = pd.to_datetime(row["timestamp"]).to_pydatetime()
            except Exception as exc:
                raise HTTPException(status_code=400, detail="timestamp 格式无效") from exc
    ...
    async def upload_vitals(self, file: UploadFile) -> int:
        ...
        for _, row in df.iterrows():
            ...
            if not (30 <= hr <= 200):
                raise HTTPException(status_code=400, detail=f"心率超出合理范围: {hr}")
            if not (5 <= br <= 40):
                raise HTTPException(status_code=400, detail=f"呼吸率超出合理范围: {br}")
            try:
                ts = pd.to_datetime(row["timestamp"]).to_pydatetime()
            except Exception as exc:
                raise HTTPException(status_code=400, detail="timestamp 格式无效") from exc
    ...
    async def upload_sleep_summary(self, file: UploadFile) -> int:
        ...
        for _, row in df.iterrows():
            ...
            try:
                ...
                raise HTTPException(status_code=400, detail="date 格式无效") from exc
            summaries.append(
                DailySleepSummary(
                    resident_id=resident_id,
                    date=d,
